...
int main(int argc, char *argv[]){
    ...
    while (1) {
        ...
        if(command_with_pipes(line)){
            sel = true;
            idx_count = 1;
            parse_command(line, args, &argsc, idx, &idx_count, &sel);
            run_pipeline(args, &argc, idx, &idx_count);

        }else if(is_cd(line)){ ///Command cd
           ...
           int status = parse_command_with_redirection(line, args, &argsc, &outfile, &infile, &append);
           if(status == 0){
